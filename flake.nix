{
  description = "Volla Cloud Services — development environment";
  ...
  # darksoil-studio/tauri-plugin-holochain, whose GitHub repo no longer
  # exists — it was renamed to p2p-shipyard — making the input unfetchable.)
  inputs = {
    ...
    inputs.flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { system, inputs', ... }:
        let
          ...
          # 28.0.13004108 is the NDK the previous dev shell shipped, so the
          # produced .so files stay the same (clang 19, 16 KB page-aligned).
          ...
          # System libraries to build a Tauri v2 app on Linux. The tauri plugin
          # crates link these on the host for `cargo check`/tests; Android builds
          # don't use them.
          tauriDeps = with pkgs; [
            webkitgtk_4_1
            gtk3
        ...
        in
        {
          devShells.default = pkgs.mkShell {
            ...
            shellHook = ''
              ...
              export PS1='\[\033[1;35m\][vcs-dev:\w]\$\[\033[0m\] '
            '';
          };
        };
    };
